...
# Connect to SharePoint using PnP Client ID (Interactive login)
$siteUrl = "https://TESTS.sharepoint.com/sites/Shop/"
Connect-PnPOnline -Url $siteUrl -ClientId 'ENTER CLIENT ID HERE' -Interactive

# Prepare output containers and log files
$report = @()                                  # To store results of items with unique permissions
$errorLogPath = "PermissionErrors.log"         # Log for errors encountered during scanning
$structureLogPath = "ScannedStructure.log"     # Log to store the full folder/file structure that was scanned
...
$totalItems = 0                                 # Total folders/files to be scanned
$currentItem = 0                                # Current item being scanned

# STEP 1: Estimate the total number of folders and files for accurate progress tracking
Write-Host "`nEstimating total items to scan..."

# Get all visible document libraries in the site (BaseTemplate 101 = Document Library)
...
foreach ($list in $lists) {
    ...
    # Use stack to simulate folder traversal (non-recursive depth-first)
    ...
    while ($stack.Count -gt 0) {
        ...
        $totalItems++                            # Count current folder
...
# STEP 2: Begin actual scanning of permissions
$totalLibraries = $lists.Count                  # Total number of libraries
$currentLibrary = 0                             # Current library index
...
foreach ($list in $lists) {
    ...
    # Check if the document library itself has unique permissions
    try {
    ...
    # Traverse all folders in the library
    $stack = New-Object System.Collections.ArrayList
    ...
    while ($stack.Count -gt 0) {
        ...
        $currentItem++                           # Increment scanned item count
        ...
        # Log the folder path being scanned
